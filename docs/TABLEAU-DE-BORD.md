...
- **Préférences générales** — devise, fuseau horaire, langue par défaut du site (utilisée quand le navigateur d'un visiteur n'indique ni français ni anglais), et les deux boutons flottants (**WhatsApp**, **assistant IA**) : décocher les retire complètement du site, pas seulement en apparence. L'assistant IA est **masqué par défaut**. Un menu déroulant permet aussi de choisir le **fournisseur IA** du chatbot (Claude, GPT, Gemini, Gemma, Grok, Kimi ou Qwen) — la clé d'API du fournisseur choisi doit être renseignée en variable d'environnement (jamais dans le tableau de bord, voir `.env.example`) ; sans clé, le chatbot continue de répondre avec ses réponses préprogrammées.
