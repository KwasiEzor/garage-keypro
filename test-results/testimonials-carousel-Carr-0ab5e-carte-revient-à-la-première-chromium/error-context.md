...
- Name: testimonials-carousel.spec.js >> Carrousel de témoignages >> la navigation boucle : "suivant" depuis la dernière carte revient à la première
- Location: tests/e2e/testimonials-carousel.spec.js:64:7

# Error details

```
Error: locator.click: Error: strict mode violation: getByRole('button', { name: /suivant|next/i }) resolved to 2 elements:
    1) <button type="button" aria-label="Témoignage suivant" class="glass absolute right-0 top-1/2 z-20 hidden h-12 w-12 -translate-y-1/2 items-center justify-center rounded-full text-white transition-all duration-300 hover:scale-105 hover:bg-white/15 sm:flex sm:-right-1 lg:right-3">…</button> aka getByRole('button', { name: 'Témoignage suivant' })
...
Call log:
  - waiting for getByRole('button', { name: /suivant|next/i })
